...
// ----------------------------------------------------------------------------
// Model4: A beam composed of 10 ANCF cable element, with one end hinged to
//         ground, moving under gravity alone.
// This model demonstrates the use of the utility class ChBuilderBeamANCF.
// ----------------------------------------------------------------------------
void model4(ChSystem& system, std::shared_ptr<ChMesh> mesh) {
	// Create a section, i.e. thickness and material properties
	// for beams. This will be shared among some beams.
	...
	builder.BuildBeam(mesh,                       // the mesh where to put the created nodes and elements
		...
		ChVector<>(3, 0, 0));  // the 'B' point in space (end of beam)

									// After having used BuildBeam(), you can retrieve the nodes used for the beam,
									// For example say you want to fix both pos and dir of A end and apply a force to the B end:
									// builder.GetLastBeamNodes().back()->SetFixed(true);
	builder.GetLastBeamNodes().front()->SetForce(ChVector<>(0, 0, 0));

	// make a box and connect it
	auto mbox2 = std::make_shared<ChBodyEasyBox>(0.2, 0.04, 0.04, 1000, true, true);
	mbox2->SetPos(builder.GetLastBeamNodes().back()->GetPos() + ChVector<>(0.1, 0, 0));
	system.Add(mbox2);
